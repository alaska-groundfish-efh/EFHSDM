...
AssembleGAMFormula<-function(yvar,gam.table,hgam=F){
  ...
  for(f in 1:length(form.list)){
    ...
    for(t in 1:nrow(g.table)){
      if(g.table$type[t]=="smooth"){out.term0<-paste0("s(",g.table$term[t])}
      if(g.table$type[t]=="factor"){out.term0<-paste0("as.factor(",g.table$term[t])}
      if(g.table$type[t]=="offset"){out.term0<-paste0("offset(",g.table$term[t])}
      
      if(is.na(g.table$term2[t])==F){out.term0<-paste0(out.term0,",",g.table$term2[t])}
      if(is.na(g.table$bs[t])==F){out.term0<-paste0(out.term0,",bs='",g.table$bs[t],"'")}
      if(is.na(g.table$k[t])==F){out.term0<-paste0(out.term0,",k=",g.table$k[t])}
      if(is.na(g.table$m[t])==F & is.na(g.table$m2[t])){out.term0<-paste0(out.term0,",m=",g.table$m[t])}
      ...
      if(g.table$type=="linear"){out.terms[t]<-g.table$term[t]}
    }
    
    if(f==1){out.form<-list(as.formula(paste0(yvar, " ~ ", paste(out.terms,collapse=" + "))))}
    if(f>1){out.form[[f]]<-as.formula(paste0(" ~ ", paste(out.terms,collapse=" + ")))}
  }
  if(
    length(form.list)==1){return(out.form[[1]])
  }else{
    return(out.form)
  }
}
...
# This function fits a gam that predicts abundance based on the data. One should  supply a formula directly
# If verbose=T, printouts of the model selection process are produced.
...
FitGAM<-function(data, 
  ...
  off.1<-unlist(lapply(strsplit(off.0,split="[()]"),FUN=function(x){return(ifelse(x[1]=="offset",x[2],NA))}))
  offset.gam<-off.1[is.na(off.1)==F]
  if(length(offset.gam)==0){offset.gam<-NULL}
  
  
  # autodetect things and construct a family, if link function isn't specified
  if(family.gam%in%c("gaussian","quasigaussian") & is.na(link.fx)){link.fx<-"identity"}
  if(family.gam%in%c("poisson","quasipoisson","nb") & is.na(link.fx)){link.fx<-"log"}
  if(family.gam=="binomial"){
    if(is.na(link.fx)){link.fx<-"logit"}
  ...
  if(select){
    ...
    if(verbose){print(summary(out.gam))}
    ...
    if(summary(out.gam)$edf[worst.var]<1){bad.vars<-worst.var}
    if(summary(out.gam)$edf[worst.var]>=1){bad.vars<-NA}
  ...
  if(reduce){
    ...
    while(reduce){
      ...
      if(verbose==T){print(summary(out.gam))}
      if(gcv_gam<=gcv_gam1){reduce<-F}
    }
  }
  return(out.gam)
}
...
FitHurdleGAM<-function(data,                                        
  ...
  if(select|reduce){
    prob.gam<-FitGAM(data=data,gam.formula = as.formula(paste0(species,as.character(prob.formula)[1],as.character(prob.formula)[2])),
    ...
    dens.gam<-FitGAM(data=data,gam.formula=dens.form0,family.gam = "poisson",reduce = reduce,
  ...
  if(reduce){
    ...
    while(reduce){
      ...
      if(prob.dens=="dens"){dens.xvars<-dens.xvars[dens.xvars$term!=drop.name,]}
      if(prob.dens=="prob"){prob.xvars<-prob.xvars[prob.xvars$term!=drop.name,]}
...
AutodetectGAMTerms<-function(model,hgam="all"){
  ...
  for(f in 1:n.formulas){
    ...
    if(model$family$family=="ziplss"){
      if(hgam%in%c("b","both","all")){form1<-formula(model)[[f]]}
      if(hgam%in%c("d","dens","density")){form1<-formula(model)[[1]]}
      if(hgam%in%c("p","prob","probability")){form1<-formula(model)[[2]]}
    }else{
      form1<-formula(model)
    }
    ...
    for(t in 1:length(terms)){
      ...
      if(length(x2)==1){
        ...
        if(x2[1]%in%c("s","te")){
          ...
          bs.spot<-which(unlist(lapply(strsplit(formula.options,"="),FUN = function(x){return(trimws(x[1]))}))=="bs")
          if(length(bs.spot)>0){
            type.dat$bs[t]<-strsplit(formula.options[bs.spot],split="\"")[[1]][2]
          }
          
          # find smoother k
          k.spot<-which(unlist(lapply(strsplit(formula.options,"="),FUN = function(x){return(trimws(x[1]))}))=="k")
          if(length(k.spot)>0){
            type.dat$k[t]<-trimws(strsplit(strsplit(formula.options[k.spot],split="=")[[1]][2],split="[)]")[[1]])
          }
          
          # find penalty m, which can be complicated
          m.spot<-which(unlist(lapply(strsplit(formula.options,"="),FUN = function(x){return(trimws(x[1]))}))=="m")
          if(length(m.spot)>0){
            if("c"%in%strsplit(formula.options[m.spot],split="")[[1]]){m.spot<-c(m.spot,m.spot+1)}
            ...
            for(n in 1:length(m.spot)){
              ...
              type.dat[t,6+n]<-na.omit(as.numeric(m3))
            }
          }
        }
        # factor terms
...
GAMStats<-function(model,                        # a gam model
  ...
  if(model$family$family=="ziplss"){
    species<-as.character(formula(model)[[1]])[[2]]
    gamfam<-"ziplss"
  }else{
    species<-as.character(formula(model))[2]
  }
  ...
  for(i in 1:length(x.vars)){
    ...
    if(gamfam=="ziplss"){
      ...
      try(new.gam<-mgcv::gam(as.formula(new.gam.form), family = gamfam, data = data))
    }
...
MakeGAMAbundance<-function(model,                 
  ...
  if("offset"%in%model.terms$type){
    ...
    off.raster<-raster(ext=r.stack@extent,crs=r.stack@crs,nrow=r.stack@nrows,ncol=r.stack@ncols,
                       vals=off.val)
  ...
  predict.raster <- predict(r.stack, model,factors=gam.factors2,progress="text",
  ...
  if(filename!=""){writeRaster(x = predict.raster,filename = filename, overwrite = TRUE)}
  
  #now apply masks if appropriate
  if(is.null(land)==F){
    predict.raster <- raster::mask(predict.raster, land, inverse = TRUE, overwrite = TRUE,
                                   filename = filename)
  }
  if(is.null(mask)==F){
    predict.raster<-raster::mask(predict.raster, mask, overwrite = TRUE,
                                 filename = filename)
  }
...
GetGAMEffects<-function(model,                          
  ...
  average.vars<-na.omit(c(var.table$term[var.table$type%in%c("factor","offset")==F],var.table$term2))
  ...
  if(nrow(vars2d)>0){
    for(v in 1:nrow(vars2d)){
      ...
      v2.data<-data.frame(x=rep(x.seq,times=40),y=rep(y.seq,each=40),
  ...
  if(nrow(vars1d)>0){
    for(v in 1:nrow(vars1d)){
      ...
      v.data<-data.frame(seq(from=min(data[,term1d]),to=max(data[term1d]),length.out=100),
      ...
      if(is.null(cv.model.list)==F){
        ...
        for(f in 1:length(cv.model.list)){
          if(is.na(cv.model.list[[f]])==F){
            cv.pred<-as.numeric(predict(cv.model.list[[f]],newdata=v.data,type="terms",
                                        terms=v.name))
            if(term1d%in%p.only){cv.pred<-cv.pred[1:nrow(v.data)]}
            if(model$family$family=="ziplss"){
  ...
  if(nrow(varsf)>0){
    for(v in 1:nrow(varsf)){
      ...
      if(is.null(cv.model.list)==F){
        ...
        for(f in 1:length(cv.model.list)){
          if(is.na(cv.model.list[[f]])==F){
            cv.pred<-as.numeric(predict(cv.model.list[[f]],newdata=f.data,type="terms",
                                        terms=fac.name))
            if(termf%in%p.only){cv.pred<-cv.pred[1:nrow(f.data)]}
            if(model$family$family=="ziplss"){
              cv.pred[-10>cv.pred]<-log(.01)
