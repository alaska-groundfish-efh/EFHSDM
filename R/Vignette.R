##########################################################################################
# This is an example vignette for the EFHSDM package. It provides an simplified example that includes
...
# For the example, we will restrict the dataset to the last ten years, note that some species were not
# adequately identified until a particular year and the dataset should be subsetted accordingly
start.year<-2012
...
########################################################################################
########################################################################################
# Now start running the maxnet models

# In practice, one should try several different values for regmult, and test the best one
# for now we'll just use 1. Also note that maxnet models frequently fail to converge.

maxnet.model<-FitMaxnet(data = species.data,species = species,
...
maxnet.cv<-CrossValidateModel(model = maxnet.model,data = species.data,species = species,group = "Folds",
...
# we could go through and get deviance and variance estimates for the model now, but it's better to
# wait and see which models actually are included in the ensemble, in order to save on processing time.
...
########################################################################################
# Poisson GAM
poisson.model<-FitGAM(gam.formula = gam.form,data = species.data,verbose = F,
...
poisson.cv<-CrossValidateModel(model = poisson.model,model.type = "gam",species = species,data = species.data,
...
negbin.model<-FitGAM(gam.formula = gam.form,data = species.data,verbose = F,select = T,
...
negbin.cv<-CrossValidateModel(model = negbin.model,model.type = "gam",species = species,data = species.data,
                              group = "Folds",key="hauljoin",scale.preds = T)
...
maxnet.variance<-MakeVarianceRasters(model.list = maxnet.cv.models,raster.stack = raster.stack,
                                     model.type = "maxnet",scale.factor = maxnet.scale)
cloglog.variance<-MakeVarianceRasters(model.list = cloglog.cv.models,raster.stack = raster.stack,
                                      model.type = "cloglog",scale.factor = cloglog.scale)
hpoisson.variance<-MakeVarianceRasters(model.list = hpoisson.cv.models,raster.stack = raster.stack,
                                       model.type = "hgam",scale.factor = hpoisson.scale)
poisson.variance<-MakeVarianceRasters(model.list = poisson.cv.models,raster.stack = raster.stack,
...
maxnet.devs<-MaxnetStats(model=maxnet.model,data = species.data,species = species,regmult = 1,
...
MakeAKGFDensityplot(region = "goa",density.map = ensemble.abund,buffer = .98,legend.title = "Abundance",
...
ensemble.effects<-GetEnsembleEffects(effects.list = effects.list,model.weights = model.weights,
                           vars = "all",scale = "log")

ensemble.plots<-Effectsplot(effects.list = ensemble.effects,region = "goa",crs = raster.stack@crs,
...
ensemble.preds<-ValidateEnsemble(pred.list = errors.list,model.weights = model.weights,output = T,make.plots = T,
...
MakeAKGFDensityplot(region = "goa",density.map = ensemble.cv,buffer = .98,legend.title = "CV of abundance",
                    title.name = figure.name)
