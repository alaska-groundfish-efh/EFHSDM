...
MakeSpeciesxtable<-function(filename,                     # a filename to write the table to
                            ...
                            error.list=NULL,              # a list with data frames containing abundance and predictions
                            weight=NULL,                  # a vector of weights to display for each model
                            scale.facs=NULL,              # a vector of scale factors
                            accs=NULL,                     # a vector of accuracys for each model
                            areas=NULL,                   # a vector of areas for each model associated EFH
                            breaks=NULL,                  # a vector of numbers that mark the minimum abundance included in EFH for each model
                            aucs=NULL){                   # a vector of auc values that associated with each model EFH
  
  n.models<-length(names)
  
  # this part detects what data are available and formats things appropriately, sturdy to missing data
  dim.names1<-""
  dim.names2<-"Model"
  
  if(is.null(error.list)==F){
    train.col<-c(length(dim.names1)+1,length(dim.names1)+2)
    test.col<-c(length(dim.names1)+3,length(dim.names1)+4)
    dim.names1<-c(dim.names1,"Training data","Training data","Testing data","Testing data")
    dim.names2<-c(dim.names2,"rsq","rmse","rsq","rmse")
  }
  if(is.numeric(weight)){
    w.col<-length(dim.names1)+1
    dim.names1<-c(dim.names1,"")
    dim.names2<-c(dim.names2,"Weight")
  }
  if(is.numeric(scale.facs)){
    sc.col<-length(dim.names1)+1
    dim.names1<-c(dim.names1,"")
    dim.names2<-c(dim.names2,"Scale Factor")
  }
  if(is.null(breaks)==F){
    break.col<-length(dim.names1)+1
    dim.names1<-c(dim.names1,"")
    dim.names2<-c(dim.names2,"EFH Break")
  }
  if(is.null(accs)==F){
    acc.col<-length(dim.names1)+1
    dim.names1<-c(dim.names1,"")
    dim.names2<-c(dim.names2,"Acc %")
  }
  if(is.null(aucs)==F){
    auc.col<-length(dim.names1)+1
    dim.names1<-c(dim.names1,"")
    dim.names2<-c(dim.names2,"AUC")
  }
  if(is.null(areas)==F){
    area.col<-length(dim.names1)+1
    dim.names1<-c(dim.names1,"")
  ...
  table1 <- array("", dim = c(length(names)+2,length(dim.names1)))
  table1[1,] <- dim.names1
  table1[2,] <- dim.names2
  
  
  # now go through and fill it in piece by piece
  for(i in 1:n.models){
    table1[i+2,1]<-names[i]
    error.dat<-error.list[[i]]
    if(is.null(error.list)==F){
      error.dat$pred[which(is.infinite(error.dat$pred))]<-10^10
      error.dat$cvpred[which(is.infinite(error.dat$cvpred))]<-10^10
      table1[i+2,train.col[1]]<-format(round(cor(error.dat$abund,error.dat$pred,method=rsq.method,)^2, 3),nsmall=3)
      table1[i+2,train.col[2]]<-format(round(RMSE(pred = error.dat$pred,obs = error.dat$abund),2),nsmall=1,big.mark = ",")
      table1[i+2,test.col[1]]<-format(round(cor(error.dat$abund,error.dat$cvpred,method=rsq.method)^2, 3),nsmall=3)
      table1[i+2,test.col[2]]<-format(round(RMSE(pred = error.dat$cvpred,obs = error.dat$abund),2),nsmall=1,big.mark = ",")
    }
    if(is.null(weight)==F){table1[i+2,w.col]<-round(weight[i],2)}
    if(is.null(scale.facs)==F){table1[i+2,sc.col]<-round(scale.facs[i],2)}
    if(is.null(breaks)==F){
      table1[i+2,break.col]<-round(breaks[i],2)
      if(is.na(breaks[i])){table1[i+2,break.col]<-"--"}
    }
    if(is.null(accs)==F){
      table1[i+2,acc.col]<-round(accs[i],3)
      if(is.na(accs[i])){table1[i+2,acc.col]<-"--"}
    }
    if(is.null(aucs)==F){
      table1[i+2,auc.col]<-round(aucs[i],3)
      if(is.na(aucs[i])){table1[i+2,auc.col]<-"--"}
    }
    if(is.null(areas)==F){
      table1[i+2,area.col]<-format(round(areas[i],-2),big.mark = ",")
      if(is.na(areas[i])){table1[i+2,area.col]<-"--"}
    }
  }
  
  spec.table <- xtable::xtable(table1)
  xtable::print.xtable(spec.table, type = "html", file = filename,
                       include.rownames = getOption("xtable.include.rownames", FALSE), html.table.attributes = 2,
                       include.colnames = getOption("xtable.include.colnames", FALSE), hline.after = getOption("xtable.hline.after",
                                                                                                               c(-1, 1, nrow(table1))))
}
